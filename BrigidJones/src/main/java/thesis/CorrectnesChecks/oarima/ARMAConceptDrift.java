...
public class ARMAConceptDrift {
    //kinda works but here OGD divergese for some reason

    public static void main(String[] args) throws IOException {
        Tuple<double[],double[]> out =  create_Series();
        new TimeSeries(out.getVal1()).writeToCSV("Datasets\\ARMAtest3.csv");
        ...
        for(int i = 0; i<10;i++) {
            ...
            a.test_accuracy("Datasets\\ARMAtest3.csv", 0.98);
            //System.out.println(m.getW());
            ARIMA_Online b = new ARIMA_Online(GridSearcher.reformat(h2));
            b.test_accuracy("Datasets\\ARMAtest3.csv", 0.98);
    ...
    private static Tuple<double[],double[]> create_Series(){
        ...
        double[] maLags;
        double[] arLags;
        RealVector a1 = MatrixUtils.createRealVector(new double[]{0.6,-0.5,0.4,-0.4,0.3});
        RealVector a2 = MatrixUtils.createRealVector(new double[]{-0.4,-0.5,0.4,0.4,0.1});
        RealVector b1 = MatrixUtils.createRealVector(new double[]{0.3,-0.2});
        RealVector b2 = MatrixUtils.createRealVector(new double[]{-0.3,0.2});
        ...
        for(int i = maxLags; i<outputArray.length;i++){
            arLags = i>(outputArray.length-maxLags)/2 ? a2.toArray() : a1.toArray();
            maLags = i>(outputArray.length-maxLags)/2 ? b2.toArray() : b1.toArray();
            double value = 0;
            for(int j = 1; j<=p;j++){
